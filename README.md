...
>   3. Tjekke at du har den nyeste version (0.9.3)
...
- **Forbedret version logging** - Integration version logger nu på WARNING niveau ved opstart
- **Meget detaljeret parsing diagnosticering** - Logger nu hver step i TimeSeries, Period og Point parsing
- **Bedre fejlsporing** - Kan nu se præcis hvilke keys der findes i hver struktur
- **Mere detaljeret logging** - Viser nu antal Periods og Points fundet, samt deres indhold
...
- **Forbedret parsing diagnosticering** - Logger nu hver step i parsing processen
- **Bedre fejlsporing** - Kan nu se præcis hvor parsing fejler og hvad der mangler
- **Mere detaljeret logging** - Viser nu data_date og antal data points efter parsing
...
- **Synlig logging** - Vigtige diagnosticerings-logs er nu på WARNING niveau så de altid vises
- **Bedre debugging** - Kan nu se præcis hvad der sker med API-kald og responses
- **Forbedret fejlsporing** - Alle vigtige logs vises nu uanset log-niveau

### Hvad er nyt i 0.8.8?

- **Forbedret fejlhåndtering** - Fanger nu exceptions fra API-kald og logger dem tydeligt
- **Bedre diagnosticering** - Logger nu response type og indhold når API returnerer data
- **Mere robust** - Håndterer nu alle typer af API-responser bedre
...
- **Synchroniseret med test.py** - Bruger nu samme dato-logik som test-scriptet der succesfuldt henter data
- **Forbedret logging** - Logger nu på INFO niveau så du kan se hvad der sker uden at aktivere debug
- **Bedre fejldiagnosticering** - Viser nu response struktur og keys når data ikke kan parses
- **Forbedret parsing** - Bedre håndtering af manglende eller uventede data strukturer
